...
A fresh checkout needs no downloads: `npm test` generates the corpus before
...
| `malformed` | Finite, valid path inputs with collapsed segments, closing residue, retracing, stationary controls, duplicate contours, arc corners and tiny genuine features |
...
`npm run pretest` first. Ordinary evaluations run in persistent child processes with a hard timeout;
a timed-out child is killed and replaced. The untimed, memory-contained mode
in [TESTING.md](../../../TESTING.md#untimed-correctness-checks) uses the same
oracles without treating elapsed time as a failure. Structural checks
...
Keep a fixture after removing its fixed failure entry. Opposite-winding copies
sharing an edge and narrow slivers, for example, now guard repairs to winding
propagation and face orientation.
